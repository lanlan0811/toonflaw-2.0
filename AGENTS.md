...
6. 提交时直接提交到 `master`，推送时使用`git push`。
7. 除非用户明确要求，否则不得新增、修改或删除 Git remote；项目默认且唯一使用现有的仓库。
8. 不得擅自主张改写 `master` 历史，包括禁止 `git push --force`、`git reset --hard` 到已发布提交及破坏性 rebase，除非是开发者本人亲自说明需要改写`master` 历史。

## 工作要求

- 修改代码前先执行 `git branch --show-current` 和 `git status --short`，确认位于 `master` 且了解现有改动。
- 保留用户已有的未提交修改，不得擅自丢弃、覆盖或回滚。
- 完成修改后按项目要求运行检查；用户要求提交和推送时，提交到 `master` 并使用`git push`推送。