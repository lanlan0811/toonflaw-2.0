...
>
> **授权状态：** 本项目维护者已获得 Toonflow 原项目权利方 HBAI-Ltd 的授权，在授权范围内进行二次开发、源码公开和版本发布；相关授权证明由本项目维护者留存。
>
...
本项目维护者已取得 Toonflow 原项目权利方 HBAI-Ltd 针对本二次开发项目的授权，可在约定范围内进行二次开发、维护、源码公开和版本发布。相关授权证明由本项目维护者留存；本声明用于说明本仓库的授权状态，不替代授权文件、`LICENSE` 或其他适用协议。

该授权不改变原项目及第三方组件的知识产权归属，也不表示本仓库属于 Toonflow 官方发行版。除非授权文件另有明确约定，该授权仅适用于本项目维护者及约定范围，不自动构成对任何第三方的商业再授权。第三方使用、修改、分发或商业部署本项目时，仍应遵守 `LICENSE`、补充协议及第三方依赖许可证；将本软件或衍生版本作为产品向两个及以上独立第三方分发、销售或提供使用前，应另行向 HBAI-Ltd 确认并取得所需书面授权。
